...
<?php
	...
	$sqli=mysqli_query($conn, "SELECT * FROM voter");
	while($leg=mysqli_fetch_array($sqli)){
		$iddd=$leg['name'];
	}
                    $sql=mysqli_query($conn, "SELECT * FROM voter where id='$idc' OR name='$iddd'");
                    $result = $conn->query($sql);

                    if ($result->num_rows > 0) {
						?>
						<script>alert('You are not allowed to vote this voter are exist')</script>
						<script>location.href='voting.php?<?php echo $idc ?>'</script>
					  <?php
											...
											<div class="row">
												<div class="col-lg-4 col-md-4 col-sm-12">
													<div class="single-tab-select-box">
														...
														<?php
																$sel=mysqli_query($conn, "SELECT * FROM candidate,voter WHERE voter.id='$idc'");
